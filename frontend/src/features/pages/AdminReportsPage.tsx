...
function shouldBlindReportedEntry(item: ReportedEntry) {
  return item.completedPlaza || item.ownerEntry;
}
        ...
        <div className="mx-auto w-full max-w-[1080px]">
          <section className="flex flex-col justify-between gap-5 border-b border-[#5a4632]/12 pb-6 sm:flex-row sm:items-end">
            <div>
              ...
              <p className="mt-2 text-sm text-[#5a4632]/58">진행 중인 일반 글은 삭제하고, 종료된 광장 또는 방장의 글은 블라인드 처리합니다.</p>
            </div>
            <button
              type="button"
                  ...
                  <div className="flex flex-col gap-4 border-b border-[#5a4632]/10 px-5 py-4 md:flex-row md:items-start md:justify-between">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2 text-xs">
                        ...
                        {item.ownerEntry && <span className="rounded-full bg-[#d39a43]/12 px-2 py-1 text-[#9a6b25]">방장 글</span>}
                    ...
                    <div className="flex shrink-0 flex-wrap gap-2 md:justify-end">
                      ...
                      >{shouldBlindReportedEntry(item) ? <EyeOff size={14} /> : <Trash2 size={14} />}{shouldBlindReportedEntry(item) ? "블라인드 및 경고" : "삭제 및 경고"}</button>
                      <button
                        type="button"
        ...
        <div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/30 px-4 py-8 backdrop-blur-[2px]" onPointerDown={() => !isActing && setActionTarget(null)}>
          <section className="w-full max-w-[460px] rounded-lg border border-[#5a4632]/15 bg-[#fffbf6] p-5 shadow-xl" onPointerDown={(event) => event.stopPropagation()}>
              ...
              <div>
                <h2 className="text-base font-semibold text-[#5a4632]">
                  {actionTarget.kind === "delete"
                    ? shouldBlindReportedEntry(actionTarget.item) ? "글을 블라인드하고 경고할까요?" : "글을 삭제하고 경고할까요?"
                    : actionTarget.kind === "suspend" ? "계정 이용을 정지할까요?" : "계정 정지를 해제할까요?"}
                </h2>
                <p className="mt-2 text-xs leading-6 text-[#5a4632]/58">
                  {actionTarget.kind === "delete"
                    ? shouldBlindReportedEntry(actionTarget.item)
                      ? "광장 기록은 보존되고, 일반 사용자에게 내용이 보이지 않게 됩니다."
            ...
            <div className="mt-5 flex justify-end gap-2">
              ...
              >
                {actionTarget.kind === "delete" ? shouldBlindReportedEntry(actionTarget.item) ? <EyeOff size={14} /> : <Trash2 size={14} /> : actionTarget.kind === "suspend" ? <Ban size={14} /> : <ShieldCheck size={14} />}
                {isActing ? "처리 중" : actionTarget.kind === "delete" ? shouldBlindReportedEntry(actionTarget.item) ? "블라인드 및 경고" : "삭제 및 경고" : actionTarget.kind === "suspend" ? "정지" : "정지 해제"}
              </button>
            </div>
          </section>
        </div>
